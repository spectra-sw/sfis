...
class MANAGER:
    def __init__(self, source= args.source, idc=args.id, urlservices= args.urlservices, params=params, vparams=vparams):
        ...
        self.IMGQUEUE  = Queue(maxsize=1)
        self.STREAMING = Queue(maxsize=1)
        # DIMENSION
        self.height = 0
        self.width  = 0
    ...
    # JSON
        ...
        for crt in annot:
            ...
            Osty = f'ANy: {Ya}'
            #-------------------------
            cv2.rectangle(imgq,(px2,py1),(px2+140,py1+60),(0,0,255),-1)
            cv2.putText(imgq, PXs,  (px2 + 5, py1 + 12 + 5), cv2.FONT_HERSHEY_PLAIN, 1.05, (0, 0, 0), 2, cv2.LINE_8 )
            cv2.putText(imgq, Ostx, (px2 + 5, py1 + 31 + 5), cv2.FONT_HERSHEY_PLAIN, 1.05, (0, 0, 0), 2, cv2.LINE_8 )
            cv2.putText(imgq, Osty, (px2 + 5, py1 + 48 + 5), cv2.FONT_HERSHEY_PLAIN, 1.05, (0, 0, 0), 2, cv2.LINE_8 )
        # Caracteristicas
        return imgq
    # STREAMING
    def STREAM(self): 
